...
class DetailpaysalariesController extends Controller
{
	/**
	...
	public function actionView($iddetail)
	{
		if(Yii::app()->authManager->checkAccess($this->formid.'-List',
            ...
            if(($model==NULL)&&isset(Yii::app()->session['Detailpaysalaries'])) {
                $model=new Detailpaysalaries;
	...
	public function actionCreate($id)
	{
             if(Yii::app()->authManager->checkAccess($this->formid.'-Append', 
                ...
                if(isset($_POST['Detailpaysalaries'])) {
                    $temp=Yii::app()->session['Detailpaysalaries'];
                    $model->attributes=$_POST['Detailpaysalaries'];
                    //posting into session
                    $temp[]=$_POST['Detailpaysalaries'];
                    
                    if (isset($_POST['yt0'])) {
	                    if ($model->validate()) {
	                        Yii::app()->session['Detailpaysalaries']=$temp;
	                        if ($master=='create')
	...
	public function actionUpdate($iddetail)
	{
                ...
                if(isset($_POST['Detailpaysalaries']))
		{
                    ...
                    foreach ($temp as $tk=>$tv) {
                        if($tv['iddetail']==$_POST['Detailpaysalaries']['iddetail']) {
                            $temp[$tk]=$_POST['Detailpaysalaries'];
                            break;
                        }
                    }
                    //posting into session
	...
	public function actionDelete($iddetail)
	{
            if(Yii::app()->authManager->checkAccess($this->formid.'-Delete', 
                ...
                foreach ($details as $ik => $iv) {
                   if($iv['iddetail']==$iddetail) {
                      if(isset(Yii::app()->session['Deletedetailpaysalaries']))
                         $deletelist=Yii::app()->session['Deletedetailpaysalaries'];
                      $deletelist[]=$iv;
                      Yii::app()->session['Deletedetailpaysalaries']=$deletelist;
                      unset($details[$ik]);
                      break;
                   }
                }
                ...
                Yii::app()->session['Detailpaysalaries']=$details;
        ...
        public function actionRestore($idtrack)
        {
            if(Yii::app()->authManager->checkAccess($this->formid.'-Update', 
               Yii::app()->user->id)) {
                ...
                $this->tracker->restore('detailpaysalaries', $idtrack);
                
                $dataProvider=new CActiveDataProvider('Detailpaysalaries');
        ...
        public function actionRestoreDeleted($idtrack)
        {
            if(Yii::app()->authManager->checkAccess($this->formid.'-Update', 
               Yii::app()->user->id)) {
                ...
                $this->tracker->restoreDeleted('detailpaysalaries', $idtrack);
	 ...
	 * @return Detailpaysalaries the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Detailpaysalaries::model()->findByPk($id);
		/*if($model===null)
	 ...
	 * @param Detailpaysalaries $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='detailpaysalaries-form')
		{
