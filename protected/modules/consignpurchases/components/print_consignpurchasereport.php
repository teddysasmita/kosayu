...
class MYPDF extends TCPDF {
	...
	public function ColoredTable() {
		...
		for ($i=0;$i<count($this->data);$i++) {
			...
			$ih = $this->getStringHeight($this->headerwidths[4],lookup::ItemNameFromItemID($row['iditem']),
					true, false, 0);
			$it = $this->getStringHeight($this->headerwidths[2],
			...
			$this->checkPageBreak($ih);
				...
				$this->total += $total;
				$this->Cell($this->headerwidths[7], $ih, number_format($total), 'BR', 0, 'R', $fill);
				$this->Cell($this->headerwidths[8], $ih, number_format($row['sellprice']), 'BR', 1, 'R', $fill);
			} 
		$this->setX(10);
	...
	public function master()
	{
		...
		$this->setFontSize(15);
		$this->setXY(10, 10);
		$this->Cell(195, 10, 'Laporan Pembelian Konsinyasi', 'LTR', 1, 'C');
		$this->SetFontSize(10);		
		$this->setX(10);
		$this->SetFont('Helvetica', 'B');
