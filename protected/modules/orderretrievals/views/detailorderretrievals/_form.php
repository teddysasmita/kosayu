...
$invdetails=json_encode(Yii::app()->session['Detailorderretrievals2']);
$itemScript=<<<EOS
	...
	function checkItems(iditem) {
      ...
      $('#Detailorderretrievals_itemname').focus(function(){
...
Yii::app()->clientScript->registerScript('itemscript', $itemScript, CClientScript::POS_READY);
 ...
 ?>
	...
	<div class="row">
		...
		<?php 
               echo CHtml::textField('Detailorderretrievals_itemname', lookup::ItemNameFromItemID($model->iditem) , array('size'=>50));   
                  ...
                  'options'=>array(
                      ...
                      'buttons'=>array(
                          array('text'=>'Ok', 'click'=>'js:function(){
                             $(\'#Detailorderretrievals_itemname\').val($(\'#dialog-item-name\').val());
                             $.get(\'index.php?r=LookUp/getItemID\',{ name: encodeURI($(\'#dialog-item-name\').val()) },
                                 function(data) {
                                    $(\'#Detailorderretrievals_iditem\').val(data);
                                 })
                             $(this).dialog("close");
                           }'),
