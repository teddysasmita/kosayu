...
class DetailpurchasesstockentriesController extends Controller
{
	/**
	...
	public function actionView($iddetail)
	{
		if(Yii::app()->authManager->checkAccess($this->formid.'-List',
            ...
            if(($model==NULL)&&isset(Yii::app()->session['Detailpurchasesstockentries'])) {
                $model=new Detailpurchasesstockentries;
                ...
                foreach ($details as $ik => $iv) {
                   if($iv['iddetail']==$iddetail) {
                      if(isset(Yii::app()->session['Deletedetailpurchasesstockentries']))
                         $deletelist=Yii::app()->session['Deletedetailpurchasesstockentries'];
                      $deletelist[]=$iv;
                      Yii::app()->session['Deletedetailpurchasesstockentries']=$deletelist;
                      unset($details[$ik]);
                      break;
                   }
                }
        ...
        public function actionRestore($idtrack)
        {
            if(Yii::app()->authManager->checkAccess($this->formid.'-Update', 
               Yii::app()->user->id)) {
                ...
                $this->tracker->restore('detailpurchasesstockentries', $idtrack);
                
                $dataProvider=new CActiveDataProvider('Detailpurchasesstockentries');
        ...
        public function actionRestoreDeleted($idtrack)
        {
            if(Yii::app()->authManager->checkAccess($this->formid.'-Update', 
               Yii::app()->user->id)) {
                ...
                $this->tracker->restoreDeleted('detailpurchasesstockentries', $idtrack);
	 ...
	 * @return Detailpurchasesstockentries the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Detailpurchasesstockentries::model()->findByPk($id);
		/*if($model===null)
	 ...
	 * @param Detailpurchasesstockentries $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='detailpurchasesstockentries-form')
		{
