...
class Detailreturstocks2Controller extends Controller
{
	/**
	...
	public function actionView($iddetail)
	{
		if(Yii::app()->authManager->checkAccess($this->formid.'-List',
            ...
            if(($model==NULL)&&isset(Yii::app()->session['Detailreturstocks2'])) {
                $model=new Detailreturstocks2;
             ...
             if(Yii::app()->authManager->checkAccess($this->formid.'-Update', 
               ...
               if(isset($_POST['yt0'])) {
                  ...
                  foreach ($temp as $tk=>$tv) {
                     if($tv['iddetail']==$_POST['Detailreturstocks2']['iddetail']) {
                         $temp[$tk]=$_POST['Detailreturstocks2'];
                         break;
                     }
                  }
                    //posting into session
                ...
                foreach ($details as $ik => $iv) {
                   if($iv['iddetail']==$iddetail) {
                      if(isset(Yii::app()->session['Deletedetailreturstocks2']))
                         $deletelist=Yii::app()->session['Deletedetailreturstocks2'];
                      $deletelist[]=$iv;
                      Yii::app()->session['Deletedetailreturstocks2']=$deletelist;
                      unset($details[$ik]);
                      break;
                   }
                }
        ...
        public function actionRestore($idtrack)
        {
            if(Yii::app()->authManager->checkAccess($this->formid.'-Update', 
               Yii::app()->user->id)) {
                ...
                $this->tracker->restore('detailreturstocks2', $idtrack);
                
                $dataProvider=new CActiveDataProvider('Detailreturstocks2');
        ...
        public function actionRestoreDeleted($idtrack)
        {
            if(Yii::app()->authManager->checkAccess($this->formid.'-Update', 
               Yii::app()->user->id)) {
                ...
                $this->tracker->restoreDeleted('detailreturstocks2', $idtrack);
	 ...
	 * @return Detailreturstocks2 the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Detailreturstocks2::model()->findByPk($id);
		/*if($model===null)
	 ...
	 * @param Detailreturstocks2 $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='detailreturstocks2-form')
		{
