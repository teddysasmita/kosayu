...
<?php
    ...
    $itemScript=<<<EOS
      $('#Detailconsignedorders_itemname').focus(function(){
         $('#ItemDialog').dialog('open');
      });
      $('#dialog-item-name').change(
         function(){
            $.getJSON('index.php?r=LookUp/getConsignedItem',{ name: $('#dialog-item-name').val() },
               function(data) {
...
EOS;
 ...
 ?>
	...
	<div class="row">
		...
		<?php 
               echo CHtml::textField('Detailconsignedorders_itemname', lookup::ItemNameFromItemID($model->iditem) , array('size'=>50));   
                  ...
                  'options'=>array(
                      ...
                      'buttons'=>array(
                          array('text'=>'Ok', 'click'=>'js:function(){
                             $(\'#Detailconsignedorders_itemname\').val($(\'#dialog-item-name\').val());
                             $.get(\'index.php?r=LookUp/getItemID\',{ name: encodeURI($(\'#dialog-item-name\').val()) },
                                 function(data) {
                                    $(\'#Detailconsignedorders_iditem\').val(data);
                                 })
                             $(this).dialog("close");
                           }'),
