...
class DetailpurchasespaymentsController extends Controller
{
	/**
	...
	public function actionView($iddetail)
	{
		if(Yii::app()->authManager->checkAccess($this->formid.'-List',
            ...
            if(($model==NULL)&&isset(Yii::app()->session['Detailpurchasespayments'])) {
                $model=new Detailpurchasespayments;
	...
	public function actionCreate($id)
	{
		...
		$this->performAjaxValidation($model);
                ...
                if(isset($_POST['Detailpurchasespayments'])) {
                    $temp=Yii::app()->session['Detailpurchasespayments'];
                    $model->attributes=$_POST['Detailpurchasespayments'];
                    //posting into session
                    $temp[]=$_POST['Detailpurchasespayments'];
                    
                    if ($model->validate()) {
                        Yii::app()->session['Detailpurchasespayments']=$temp;
                        if ($master=='create')
             ...
             if(Yii::app()->authManager->checkAccess($this->formid.'-Update', 
               ...
               if(isset($_POST['Detailpurchasespayments'])) {
                  ...
                  foreach ($temp as $tk=>$tv) {
                     if($tv['iddetail']==$_POST['Detailpurchasespayments']['iddetail']) {
                         $temp[$tk]=$_POST['Detailpurchasespayments'];
                         break;
                     }
                  }
                    //posting into session
        ...
        public function actionRestore($idtrack)
        {
            if(Yii::app()->authManager->checkAccess($this->formid.'-Update', 
               Yii::app()->user->id)) {
                ...
                $dataProvider=new CActiveDataProvider('Detailpurchasespayments');
	 ...
	 * @return Detailpurchasespayments the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Detailpurchasespayments::model()->findByPk($id);
		/*if($model===null)
	 ...
	 * @param Detailpurchasespayments $model the model to be validated
	 */
