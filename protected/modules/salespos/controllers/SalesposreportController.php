...
EOS;
	...
	a.price, a.discount, e.firstname
...
EOS;
				...
				$summarysales = Yii::app()->session['datasales3'];
			};								
				...
				foreach($datasales as $ds1) {
					...
					foreach($summarysales as &$ss) {
						if ($ss['batchcode'] == $batchcode) {
							$ss['qty'] += $ds1['qty'];
							$ss['rqty'] += $ds1['rqty'];
							$ss['totalsold'] += ($ds1['qty'] - $ds1['rqty']) * $ds1['price'];
							...
							$ss['sellprice'] = lookup::getsellprice($ss['batchcode']);
							$found = TRUE;
							break;
						}
					}
