...
class ReceiptsController extends Controller
{
	...
	{     
            ...
            if (!isset(Yii::app()->session['Receipts'])) {
               $this->onInsertMaster($model);
               Yii::app()->session['Receipts']=$model->attributes;
            }  
		...
		// $this->performAjaxValidation($model);
            ...
            if (isset($_POST)){
               if(isset($_POST['yt0']))
               {
                  ...
                  $details=Yii::app()->session['Detailreceipts'];
                  
                  if($model->save()&&$this->saveNewDetails($details)) {
                     $idmaker=new idmaker();
                     $idmaker->saveRegNum($this->formid, $model->regnum);
                     Yii::app()->session->remove('Receipts');
                     Yii::app()->session->remove('Detailreceipts');
                  ...
                  if($_POST['command']=='adddetail') {
                     $model->attributes=$_POST['Receipts'];
                     Yii::app()->session['Receipts']=$_POST['Receipts'];
                     $this->redirect(array('detailreceipts/create','id'=>$model->id));
                  }
               }
            }
	...
	public function actionUpdate($id)
	{
		$model=$this->loadModel($id);
            ...
            if (!isset(Yii::app()->session['Receipts'])&&
                !isset(Yii::app()->session['Detailreceipts'])) {
               Yii::app()->session['Receipts']=$model->attributes;
               Yii::app()->session['Detailreceipts']=$this->loadDetails($id);
            }
		...
		if(isset($_POST)) {
			...
			$model->attributes=$_POST['Receipts'];
                  $details=Yii::app()->session['Detailreceipts'];
                  $respond=true;
                  if (isset(Yii::app()->session['Deletedetailreceipts'])) {
                     $deletedetails=Yii::app()->session['Deletedetailreceipts'];
                  ...
                  if($model->save()&&$this->saveDetails($details)
                     ...
                     Yii::app()->session->remove('Receipts');
                     Yii::app()->session->remove('Detailreceipts');
                     Yii::app()->session->remove('Deletedetailreceipts');
                     $this->redirect(array('view','id'=>$model->id));
                  }
               }
		}
	...
	protected function performAjaxValidation($model)
	{
      ...
      public function actionCreateDetail()
      {
         ...
         $model=new Receipts;
         $model->attributes=Yii::app()->session['Receipts'];
      ...
      protected function saveNewDetails(array $details)
      {
         ...
         foreach ($details as $row) {
            $detailmodel=new Detailreceipts;
      ...
      protected function saveDetails(array $details)
      {
         ...
         foreach ($details as $row) {
            $detailmodel=Detailreceipts::model()->findByPk($row['iddetail']);
            if($detailmodel==NULL)
               $detailmodel=new Detailreceipts;
            $detailmodel->attributes=$row;
      ...
      {
         $sql="select * from detailreceipts where id='$id'";
